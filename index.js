...
(function rendering() {
    ...
    elements.forEach(i => {
        i.addEventListener("pointerup", function () {
            ...
            renderPrice();
            document.getElementById(`${i.parentElement.id}_i`).classList.toggle('open_i');
        });
    })
})();

let $span = document.getElementById("finaly_price_span");
let $message = document.getElementById("after_result");

function renderPrice() {
    if (!parameters.popUp_color || !parameters.popUp_pleace) result = '---';
    else if (square <= 16) {
        result = Math.ceil((2500 * parameters.popUp_color) / 50) * 50;
        $message.style.display = 'block';
    } else {
        result = Math.ceil(
            (((square - 16) * parameters.popUp_pleace + 2500) * parameters.popUp_color) / 50
        ) * 50;
        $message.style.display = 'block';
    }
